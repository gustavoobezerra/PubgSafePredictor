...
public class PUBGSafeZonePredictor {
    ...
    private static final double[][][] PROBABILITIES = {
        ...
        {
            {10, 27, 30},  // Zona Central (sobre, próximo, distante)
            {5, 12, 15},    // Zona Periférica
            {0, 1, 0}       // Zona de Borda
        },
        // Rota atravessa apenas Zona Periférica
        {
            {5, 17, 25},//zona central
            {8, 20, 22},//zona periferica
            {1, 2, 0}//zona DE borda 
        },
...
drawingPanel.addMouseListener(new MouseAdapter() {
            ...
            public void mousePressed(MouseEvent e) {
                ...
                try {
                    BufferedImage mapImg = mapImages.get(MAPS[selectedMap]);
                    if (mapImg != null) {
                        // Pega a cor exata onde o mouse clicou
                        Color color = new Color(mapImg.getRGB(e.getX(), e.getY()));
                        
                        // Imprime os valores R, G, B no console
                        System.out.println(
                            "DEBUG COR NO PONTO (" + e.getX() + "," + e.getY() + "): " + 
                            "R=" + color.getRed() + ", " +
                            "G=" + color.getGreen() + ", " +
                            "B=" + color.getBlue()
                        );
                    ...
                    // Ignora o erro se clicar fora da imagem 
                }
    ...
    // Lista dos nomes dos mapas para facilitar o loop
    String[] mapNames = {"erangel", "miramar", "taego", "rondo"};
    
    try {
        for (String mapName : mapNames) {
            // Constrói o caminho do recurso. O "/" no início é importante!
            String path = "/safePubg/maps/" + mapName + ".png"; 
            
            // Tenta carregar a imagem como um recurso do sistema
            java.io.InputStream imageStream = PUBGSafeZonePredictor.class.getResourceAsStream(path);
            
            if (imageStream == null) {
                // Se não encontrar, lança uma exceção para ser tratada abaixo
                throw new IOException("Recurso não encontrado: " + path);
            }
            
            // Transforma o fluxo de dados em uma BufferedImage
            BufferedImage img = ImageIO.read(imageStream);
            
            // Capitaliza o nome do mapa para usar como chave no HashMap (ex: "Erangel")
        ...
        // Se falhar, cria as imagens padrão
    ...
    */
    // Este é o novo método generateZonePoint
/*private static Point2D generateZonePoint(int zone, int distance) {
    Random rand = new Random();
    double centerX = 400; // Assumindo panel width is 800
    double centerY = 300; // Assumindo panel height is 600
    double mapRadius = 400;
    
    // Pega a imagem do mapa que está selecionado no momento
    ...
    do {
        ...
        // --- NOVA LÓGICA DE DETECÇÃO DE ÁGUA ---
        boolean isLand = true; // Começamos assumindo que é terra
        if (currentMapImage != null) {
            ...
            if (px >= 0 && px < currentMapImage.getWidth() && py >= 0 && py < currentMapImage.getHeight()) {
                ...
                // Pega a cor do pixel no mapa original
                Color pixelColor = new Color(currentMapImage.getRGB(px, py));
                int red = pixelColor.getRed();
                int green = pixelColor.getGreen();
                int blue = pixelColor.getBlue();
                
                // CONDIÇÃO SIMPLES: Se o azul for muito mais forte que o verde e o vermelho,
                // provavelmente é água.
                // (Você pode ajustar esses números se não funcionar bem)
                if (blue > 120 && blue > (red + 30) && blue > (green + 30)) {
                    isLand = false; // É água! Rejeita o ponto.
                }
            }
        }
        // --- FIM DA NOVA LÓGICA ---
*/
        // Calculate distance to route
        // Este é o novo método generateZonePoint CORRIGIDO
private static Point2D generateZonePoint(int zone, int distance) {
    Random rand = new Random();
    double centerX = 400; // Assumindo panel width is 800
    double centerY = 300; // Assumindo panel height is 600
    ...
    do {
        ...
        // --- NOVA LÓGICA DE DETECÇÃO DE ÁGUA (BASEADA NOS SEUS DADOS) ---
        boolean isLand = true; // Começamos assumindo que é terra
        if (currentMapImage != null) {
            int px = (int) point.getX();
            int py = (int) point.getY();
            ...
            if (px >= 0 && px < currentMapImage.getWidth() && py >= 0 && py < currentMapImage.getHeight()) {
                ...
                // **** A NOSSA NOVA REGRA ****
                // Se Azul > Verde E Azul > (Vermelho + 10), então é ÁGUA.
        ...
        // Check if distance matches AND if it's land
        // SÓ RETORNA SE AMBAS AS CONDIÇÕES FOREM VERDADEIRAS
        if (isLand && distToRoute >= minDistFromRoute && distToRoute <= maxDistFromRoute) {
            return point;
        }
        
        attempts++;
    } while (attempts < 2000); // Aumentei para 2000 tentativas
    
    // Se não conseguir achar um ponto perfeito (terra + distância certa),
    // tenta achar um ponto que seja PELO MENOS em terra.
    attempts = 0; // Reseta o contador
    do {
        double angle = rand.nextDouble() * 2 * Math.PI;
        double radius = innerRadius + rand.nextDouble() * (outerRadius - innerRadius);
        double x = centerX + radius * Math.cos(angle);
        ...
        if(isLand) return point; // Retorna o primeiro ponto em terra que achar
        
        attempts++;
    } while (attempts < 5000); // Tenta mais 5000 vezes

    // Se falhar TUDO, só retorna o último ponto que tentou (pode ser na água)
    // Isso é melhor do que retornar um ponto totalmente aleatório
    System.out.println("AVISO: Não foi possível encontrar um ponto ideal em terra. Retornando último ponto.");
    return point;
 }
